...
public class SummaryServiceImpl implements SummaryService {
	...
	@Override
	public Object fetchTotalGroupExpense(Long groupId, CustomUserDetails userDetails) throws CommonException {
		try {
			businessValidationService.validate(Map.of(Constants.VALIDATION_USER_GROUP,
					new UserGroupDto(groupId, null, List.of(userDetails.getUserId()))));
			...
			LOGGER.error("Exception in fetchTotalGroupExpense", e);
			if (e instanceof CommonException ce)
				throw ce;
			else
				throw new CommonException(FailureConstants.FETCH_TOTAL_GROUP_EXPENSE_SUMMARY.getFailureCode(),
						FailureConstants.FETCH_TOTAL_GROUP_EXPENSE_SUMMARY.getFailureMsg());
		}
	}
